...
test.describe("Medical Exam - Patient view", () => {
  test("Patient views exam result or pending notice", async ({ page }) => {
    ...
    const hasExam = await page.getByText(/Kết quả khám bệnh/i).isVisible().catch(() => false);
    if (hasExam) {
      await expect(page.getByText(/Đơn thuốc|Phác đồ điều trị|Chẩn đoán/i)).toBeVisible();
    } else {

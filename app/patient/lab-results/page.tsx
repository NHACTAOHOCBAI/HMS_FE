...
import { LabTestResult, ResultStatus, LabTestCategory, labResultService } from "@/services/lab.service";
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-6">
      {results.map((result) => {
        ...
        return (
          <Link key={result.id} href={`/patient/lab-results/${result.id}`}>
            <Card
...
export default function PatientLabResultsPage() {
  ...
  const { data: results, isLoading, refetch, isFetching } = useQuery({
    queryKey: ["labResults", "patient", patientId],
  ...
  const filteredResults = useMemo(() => {
    ...
    return filtered;
  }, [labResults, statusFilter, abnormalOnly, debouncedSearch, dateRange]);

  // Calculate stats
  const stats = useMemo(() => {
    const pending = labResults.filter((r: LabTestResult) => r.status === "PENDING").length;
    const processing = labResults.filter((r: LabTestResult) => r.status === "PROCESSING").length;
    const completed = labResults.filter((r: LabTestResult) => r.status === "COMPLETED").length;
    const abnormal = labResults.filter((r: LabTestResult) => r.isAbnormal).length;
    return { total: labResults.length, pending, processing, completed, abnormal };
  }, [labResults]);
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight flex items-center gap-2">
                Kết quả Xét nghiệm
                {isFetching && <RefreshCcw className="h-4 w-4 animate-spin" />}
              </h1>
              <p className="mt-1 text-cyan-100">
                Xem tất cả kết quả xét nghiệm của bạn
              </p>
            </div>
          </div>
      ...
      <div className="space-y-3 rounded-xl bg-slate-50/80 p-4 border border-slate-200 shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative flex-1 min-w-[200px] ml-auto">
            ...
            <Input
              placeholder="Tìm theo tên XN, mã XN..."
              value={search}
              onChange={(e) => {

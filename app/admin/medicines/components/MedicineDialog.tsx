...
const MedicineDialog = ({
  ...
  setEditingMedicine: React.Dispatch<
    React.SetStateAction<MedicineResponse | null>
  >;
  ...
  const onMedicineSubmit: SubmitHandler<z.infer<typeof medicineFormSchema>> = (
    values
  ) => {
  ...
  useEffect(() => {
    if (editingMedicine) {
      ...
      medicineForm.setValue(
        "activeIngredient",
        editingMedicine.activeIngredient
      );
      medicineForm.setValue(
        "categoryId",
        editingMedicine.category?.id.toString() || ""
      );
      ...
      medicineForm.setValue(
        "expiresAt",
        editingMedicine.expiresAt.split("T")[0]
      );

...
import { getDepartments, getPositions, getAllEmployees, getEmployeeSchedule, addEmployeeToShift, removeEmployeeFromShift } from "@/services/employee.service";
import { ChevronLeft, ChevronRight, Calendar, X, User, Users } from "lucide-react";
...
export default function SchedulingPage() {
    ...
    const handleDropOnShift = async (e: React.DragEvent, day: string, shiftName: string) => {
        e.preventDefault();
    ...
    const handleRemoveFromShift = async (day: string, shiftName: string, employeeId: number) => {
    ...
    return (
        <div className="p-6">
            <div className="mb-6 flex items-center gap-3">
                ...
                <div className="flex items-center gap-2 ml-4 border rounded-lg px-3 py-2">
                    <Button variant="ghost" size="icon" onClick={handlePreviousWeek} className="cursor-pointer">
            ...
            <div className="flex gap-6">
                                        ...
                                        {weekDays.map((day, dayIdx) => {
                                            ...
                                            return (
                                                <td
                                                    ...
                                                    onDrop={(e) => handleDropOnShift(e, day.fullDateStr, shift.name)}
                                                >
                                                    {shiftSchedule ? (
                                                        <div className="space-y-1">
                                                            {shiftSchedule.employees.map((emp: any, empIdx: number) => (
                                                                <div
                                                                    key={empIdx}
                                                                ...
                                                                >
                                                                    <span className="flex-1">{emp.fullName.split(" ").slice(-2).join(" ")}</span>
                                                                    <button
                                                                        onClick={() => handleRemoveFromShift(day.fullDateStr, shift.name, emp.id)}
                ...
                <div className="w-64 border rounded-lg p-4">
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        <h3 className="font-semibold">Employee lists</h3>
                    </div>
                        ...
                        {filteredEmployees.length > 0 ? (
                            filteredEmployees.map((emp, idx) => (
                                <div
                                    ...
                                    className={`p-2 border rounded-lg hover:bg-gray-50 cursor-grab active:cursor-grabbing transition-all ${draggedEmployee?.id === emp.id ? "opacity-50 scale-95" : ""
                                        }`}
                                >
